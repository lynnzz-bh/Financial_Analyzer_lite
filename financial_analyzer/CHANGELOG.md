...
- 修正 PE 口径：东方财富/AKShare 的 `市盈率-动态` 单独保存为 `PE 动态`，不再冒充 `PE TTM`；真正的 `PE TTM` 改为由总市值和 TTM 归母净利润计算。
- 修正 PEG 口径：PEG 使用项目内部计算的真实 `PE TTM`，不再使用行情源动态市盈率作为分子。
- 修正 PB 口径：东方财富/AKShare 的市净率字段单独保存为 `PB 行情源`，主 `PB` 改为由总市值和最新资产负债表股东权益计算。
- 报告估值章节同时展示 `PE 动态`、`PE TTM`、`PB 行情源` 和项目计算 `PB`，便于对照行情源字段与财报口径。
- provenance 同步更新 PE/PB 来源说明：`PE TTM` 追溯到 `market_data: 总市值` 和 TTM 归母净利润，`PB` 追溯到 `market_data: 总市值` 和最新资产负债表股东权益。
- 增加行情字段映射测试、年度/季度 ROE 拆分测试、半年度前缀测试、PE/PB 项目计算口径测试，防止再次把动态/行情源字段误标为 TTM 或主指标。
